...
export default function VoiceButton({ apiKey, onSearchResults }: VoiceButtonProps) {
  ...
  if (!supported) {
    return (
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 px-5 py-3 text-xs sm:text-sm text-red-300 bg-[#202e26] border border-red-500/30 rounded-2xl shadow-2xl z-50">
  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2.5 z-40 px-4 w-full max-w-md pointer-events-none">
      ...
      >
        <div className="voice-island px-5 py-3 text-center text-[#f2efe9] border border-white/20 shadow-2xl flex items-center justify-center gap-2.5">
          {listening && !transcript && (
            <div className="flex items-center gap-2.5">
              <div className="flex items-center gap-1 h-5">
                <span className="w-1 bg-[#dedbd2] rounded-full bar-1" />
                <span className="w-1 bg-[#dedbd2] rounded-full bar-2" />
                <span className="w-1 bg-[#dedbd2] rounded-full bar-3" />
                <span className="w-1 bg-[#dedbd2] rounded-full bar-4" />
              </div>
              <span className="text-xs font-semibold uppercase tracking-wider text-[#dedbd2]">
                Listening…
              </span>
            </div>
          )}
          ...
          {transcript && !isProcessing && (
            <span className="text-xs sm:text-sm font-medium text-[#f2efe9] truncate max-w-xs">
              &ldquo;{transcript}&rdquo;
            </span>
          )}

          {isProcessing && (
            <div className="flex items-center gap-2 text-[#dedbd2] text-xs sm:text-sm font-medium">
          ...
          {!isProcessing && !transcript && feedback && (
            <div className="flex items-center gap-2">
              <Volume2 className="w-3.5 h-3.5 text-[#dedbd2] shrink-0" />
              <span className="text-xs sm:text-sm font-medium text-[#f2efe9]">
                {feedback}
              </span>
              {usedAi && (
                <span className="px-1.5 py-0.5 rounded bg-white/10 text-[#dedbd2] text-[10px] font-bold tracking-wider uppercase font-mono">
                  AI
                </span>
              )}
            </div>
      ...
      <div className="voice-island p-1.5 flex items-center gap-2 pointer-events-auto border border-white/20 shadow-2xl bg-[#1d2b22]/95">
        {/* Language Switcher */}
        <div className="flex bg-black/30 rounded-full p-0.5 border border-white/10">
          {LANGUAGES.map((l) => (
            <button
              key={l.code}
              ...
              className={`px-3 py-1 rounded-full text-xs font-semibold transition-all ${
                lang === l.code
                  ? "bg-[#dedbd2] text-[#202922] font-bold shadow"
        ...
        {apiKey && (
          <span
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/10 text-[#dedbd2] border border-white/15 text-[10px] font-semibold"
          >
        ...
        <div className="relative">
          {listening && (
            <span
              className="absolute inset-[-4px] rounded-full bg-white/20 botanical-pulse-active"
              aria-hidden
            />
          )}
          <button
            ...
            className={`relative w-11 h-11 rounded-full flex items-center justify-center transition-all ${
              listening
                ...
                : "bg-[#50775a] hover:bg-[#5d8b69] text-white hover:scale-105 active:scale-95 shadow-lg"
